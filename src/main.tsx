...
function Root() {
  ...
  const [page, setPage] = useState<'dashboard' | 'focus' | 'syllabus' | 'ai'>(() => window.location.hash === '#focus' ? 'focus' : window.location.hash === '#syllabus' ? 'syllabus' : window.location.hash === '#ai' ? 'ai' : 'dashboard');
  ...
  useEffect(() => { const navigate = (event: MouseEvent) => { const link = (event.target as HTMLElement).closest('a'); const target = link?.textContent?.trim(); const next = target === 'Focus Tools' ? 'focus' : target === 'AI Study Assistant' ? 'ai' : target === 'Full Syllabus' ? 'syllabus' : target === 'Overview' ? 'dashboard' : undefined; if (next) { event.preventDefault(); window.location.hash = next === 'dashboard' ? '' : next; setPage(next); } }; document.addEventListener('click', navigate); return () => document.removeEventListener('click', navigate); }, []);
  const content = page === 'focus' ? <FocusTools /> : page === 'syllabus' ? <Syllabus /> : page === 'ai' ? <AiTutor /> : <><Planner /><Strategy /><Tasks /></>;
